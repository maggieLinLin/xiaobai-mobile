...
function renderMiniCalendar() {
    ...
    if (todayMemoWidget) {
        ...
        const text = memo && memo.trim() ? memo : '今天没有任务';

        if (todayMemoWidget.tagName === 'TEXTAREA') {
            // 让 calendar-integration 负责事件与高度，避免相互覆盖
            todayMemoWidget.value = text;
            todayMemoWidget.dispatchEvent(new Event('input', { bubbles: true }));
        } else {
            todayMemoWidget.textContent = text;
        }
    }
...
function saveMemo() {
    const date = state.selectedDate || getLocalDateKey();
